...
play.init = function (){
	...
	play.labelDoing		=	"red";				//可以下棋的是哪一方，默认为红方，值只能有red和black
	//初始化棋子
...
play.clickMan = function (key,x,y){
	...
	if (play.nowManKey&&play.nowManKey != key && man.my != com.mans[play.nowManKey ].my){
		...
		if (play.indexOfPs(com.mans[play.nowManKey].ps,[x,y])){
			...
			play.setLabelDoing(key[0]);
			if (key == "j0") play.showWin (-1);
			if (key == "J0") play.showWin (1);
		}
	// 选中棋子
	}else{
		if (play.whichLabel(key[0])){
...
//设置下一步是哪一方走
play.setLabelDoing = function(key){
	var red = "['c','m','x','s','j','p','z']";
	var black = "['C','M','X','S','J','P','Z']";
	for( k in red )
	{
		if( red[k] == key )
		{
			play.labelDoing = "black";
			break;
		}
	}
	for( k in black )
...
//判断哪一方可以选中棋子
play.whichLabel = function(key){
	var red = "['c','m','x','s','j','p','z']";
	var black = "['C','M','X','S','J','P','Z']";
	if( play.labelDoing == "red" )
	{
		for( k in red )
		{
			if( red[k] == key )
