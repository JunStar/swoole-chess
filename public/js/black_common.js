...
com.loadImages = function(stype){
	...
	com.dotImg.src  = SITE_URL+"/public/img/"+stype+"/dot.png";

	//棋子
	for (var i in com.args){
		com[i] = {};
		com[i].img = new Image();
		com[i].img.src = SITE_URL+"/public/img/"+stype+"/"+ com.args[i].img +".png";
	}

	//棋子外框
	com.paneImg 	= new Image();
	com.paneImg.src = SITE_URL+"/public/img/"+stype+"/r_box.png";
...
com.createMove = function (map,x,y,newX,newY){
	...
	if( type == "退" || type == "进" )
	{
		new_map = [
			...
			[ map[0][1], map[1][1], map[2][1], map[3][1], map[4][1], map[5][1], map[6][1], map[7][1], map[8][1], map[9][1] ],
			[ map[0][2], map[1][2], map[2][2], map[3][2], map[4][2], map[5][2], map[6][2], map[7][2], map[8][2], map[9][2] ],
			[ map[0][3], map[1][3], map[2][3], map[3][3], map[4][3], map[5][3], map[6][3], map[7][3], map[8][3], map[9][3] ],
			[ map[0][4], map[1][4], map[2][4], map[3][4], map[4][4], map[5][4], map[6][4], map[7][4], map[8][4], map[9][4] ],
			[ map[0][5], map[1][5], map[2][5], map[3][5], map[4][5], map[5][5], map[6][5], map[7][5], map[8][5], map[9][5] ],
			[ map[0][6], map[1][6], map[2][6], map[3][6], map[4][6], map[5][6], map[6][6], map[7][6], map[8][6], map[9][6] ],
			[ map[0][7], map[1][7], map[2][7], map[3][7], map[4][7], map[5][7], map[6][7], map[7][7], map[8][7], map[9][7] ],
			[ map[0][8], map[1][8], map[2][8], map[3][8], map[4][8], map[5][8], map[6][8], map[7][8], map[8][8], map[9][8] ]
		]

		for( new_map_x_key in new_map[x] )
		{
			if( new_map[x][new_map_x_key] )
			{
				if( new_map[x][new_map_x_key][0] == key[0] )
				{
					new_x_same.push( new_map[x][new_map_x_key] );
				}
			}
		}

		if( new_x_same.length == 2 )
		{
			if( new_x_same[0] == key )
			{
				h = "后" + h;
